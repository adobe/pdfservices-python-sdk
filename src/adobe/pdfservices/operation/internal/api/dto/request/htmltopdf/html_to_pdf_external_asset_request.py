...
from adobe.pdfservices.operation.internal.api.dto.request.htmltopdf.html_to_pdf_params_payload import \
    HTMLtoPDFParamsPayload
...
class HTMLtoPDFExternalAssetRequest(PDFServicesAPIRequest):
    json_hint = {
        'input': 'input',
        'output': 'output',
        'params': 'params',
    ...
    def __init__(self, input_asset: Asset, html_to_pdf_params: HTMLtoPDFParams,
        ...
        if html_to_pdf_params is not None:
            self.params: HTMLtoPDFParamsPayload = HTMLtoPDFParamsPayload(html_to_pdf_params)
