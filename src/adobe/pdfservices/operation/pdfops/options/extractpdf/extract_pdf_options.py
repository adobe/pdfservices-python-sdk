...
from adobe.pdfservices.operation.pdfops.options.extractpdf.pdf_element_type import PDFElementType
...
class ExtractPDFOptions():
    ...
    class Builder:
        ...
        def with_element_to_extract(self, element_to_extract: PDFElementType):
            """
            adds a pdf element type for extracting structured information.

            :param element_to_extract: PDFElementType to be extracted
            :type element_to_extract: PDFElementType
            :return: This Builder instance to add any additional parameters.
            :rtype: ExtractPDFOptions.Builder
            :raises ValueError: if element_to_extract is None.
            """
            if element_to_extract and element_to_extract in PDFElementType:
                self._init_elements_to_extract()
                self._elements_to_extract.append(element_to_extract)
            else:
                raise ValueError("Only PDFElementType enum is accepted for element_to_extract")
            return self

        def with_elements_to_extract(self, elements_to_extract: List[PDFElementType]):
            """
            adds a list of pdf element types for extracting structured information.

            :param elements_to_extract: List of PDFElementType to be extracted
            :type elements_to_extract: List[PDFElementType]
            :return: This Builder instance to add any additional parameters.
            :rtype: ExtractPDFOptions.Builder
            :raises ValueError: if elements_to_extract is None or empty list.
            """
            if elements_to_extract and all(element in PDFElementType for element in elements_to_extract):
                self._init_elements_to_extract()
                self._elements_to_extract.extend(elements_to_extract)
            else:
                raise ValueError("Only PDFElementType enum List is accepted for elements_to_extract")
            return self

        def with_element_to_extract_renditions(self, element_to_extract_renditions: PDFElementType):
            """
            adds a pdf element type for extracting rendition.

            :param element_to_extract_renditions: PDFElementType whose renditions have to be extracted
            :type element_to_extract_renditions: PDFElementType
            :return: This Builder instance to add any additional parameters.
            :rtype: ExtractPDFOptions.Builder
            :raises ValueError: if element_to_extract_renditions is None.
            """
            if element_to_extract_renditions and element_to_extract_renditions in PDFElementType:
                self._init_elements_to_extract_renditions()
                self._elements_to_extract_renditions.append(element_to_extract_renditions)
            else:
                raise ValueError("Only PDFElementType enum is accepted for element_to_extract_renditions")
            return self

        def with_elements_to_extract_renditions(self, elements_to_extract_renditions: List[PDFElementType]):
            """
            adds a list of pdf element types for extracting rendition.

            :param elements_to_extract_renditions: List of PDFElementType whose renditions have to be extracted
            :type elements_to_extract_renditions: List[PDFElementType]
            :return: This Builder instance to add any additional parameters.
            :rtype: ExtractPDFOptions.Builder
            :raises ValueError: if elements_to_extract is None or empty list.
            """
            if elements_to_extract_renditions and all(
                    element in PDFElementType for element in elements_to_extract_renditions):
                self._init_elements_to_extract_renditions()
                self._elements_to_extract_renditions.extend(elements_to_extract_renditions)
            else:
                raise ValueError("Only PDFElementType enum List is accepted for elements_to_extract_renditions")
            return self
